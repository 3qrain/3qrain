...
<template>
  <div class="page">
    <div class="head">
      <div>
        <h1>标签</h1>
        <span class="sub">{{ list.length }} 个标签</span>
      </div>
    ...
    <Transition name="slide">
      <div v-if="adding" class="inline-form">
        <div class="fields">
          <input v-model="form.name" class="field" placeholder="名称，如：TypeScript" @keyup.enter="save" @keyup.esc="cancel" />
          <input v-model="form.slug" class="field slug" placeholder="标识，如：typescript" @keyup.enter="save" @keyup.esc="cancel" />
        </div>
